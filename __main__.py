...
account_id = config.get_secret('account_id')
...
task = aws.ecs.TaskDefinition(project_stack,
    cpu='256',
    execution_role_arn=account_id.apply(lambda arg: f'arn:aws:iam::{arg}:role/ecsTaskExecutionRole'),
    family=project_stack,
    memory='512',
    network_mode='awsvpc',
    requires_compatibilities=['FARGATE'],
    task_role_arn=account_id.apply(lambda arg: f'arn:aws:iam::{arg}:role/AWSECSTaskRole'),
...
event_rule = aws.cloudwatch.EventRule(project_stack,
    ...
    role_arn=account_id.apply(lambda arg: f'arn:aws:iam::{arg}:role/ecsTaskExecutionRole'),
    is_enabled=True,
    tags={'group': 'sdsnv2020', 'stack': stack},
)
...
event_target = aws.cloudwatch.EventTarget(project_stack,
    ...
    role_arn=account_id.apply(lambda arg: f'arn:aws:iam::{arg}:role/ecsEventsRole'),
